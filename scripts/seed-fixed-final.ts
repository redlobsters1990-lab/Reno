// Final Fixed Seed Script
// Compatible with updated Prisma schema
...
async function seedFixedFinal() {
  console.log('=== FINAL FIXED SEED SCRIPT ===');
  ...
  try {
    ...
    // Create projects for standard user (if not exists)
    console.log('\n2. Creating test projects...');
    const standardUser = users.find(u => u.email === 'standard@example.com');
    if (standardUser) {
      // Check if project already exists
      const existingProject = await prisma.project.findFirst({
        where: { userId: standardUser.id, title: 'Kitchen Renovation' }
      });
      ...
      if (!existingProject) {
        ...
        // Create cost estimate with correct schema
        const estimate = await prisma.costEstimate.create({
          data: {
            ...
            costDrivers: JSON.stringify(['cabinets', 'countertops', 'appliances', 'labor']),
        ...
        console.log(`  ✓ Created cost estimate: $${estimate.leanMin}-$${estimate.stretchMax}`);
        ...
        const job = await prisma.job.create({
          data: {
            ...
            output: JSON.stringify({ 
              estimate: '15000-35000', 
              confidence: 'medium',
              breakdown: {
                cabinets: '5000-12000',
                countertops: '2000-6000',
    ...
    if (heavyUser) {
      ...
      if (existingProjects < 4) {
        ...
        console.log(`  ✓ Heavy user already has ${existingProjects} projects`);
      }
    }
